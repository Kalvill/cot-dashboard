...
Логін / реєстрація / збереження bias + report stances
"""
...
# DB_PATH: спочатку пробуємо Docker-шлях, якщо не існує — локальний
_docker_db = Path("/app/data/users.db")
DB_PATH = _docker_db if _docker_db.parent.exists() else BASE_DIR / "users.db"
...
def init_db():
    ...
    with get_db() as db:
        db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS rpt_stances (
                id         INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id    INTEGER NOT NULL,
                instrument TEXT    NOT NULL,
                ...
                updated    DATETIME DEFAULT CURRENT_TIMESTAMP,
                UNIQUE(user_id, instrument, report),
                FOREIGN KEY(user_id) REFERENCES users(id)
            );
        """)
...
# ================================================================
# 📊  API — BIAS (загальний по інструменту)
# ================================================================
...
def get_all_bias():
    ...
    return jsonify({r['instrument']: {'value': r['value'], 'note': r['note']} for r in rows})
...
# ================================================================
# 📋  API — RPT_STANCES (L/N/S по звітах для кожного інструменту)
# ================================================================
@app.route('/api/rpt_stance', methods=['POST'])
@login_required
def save_rpt_stance():
    """Зберігає stance (long/short/neutral) для конкретного звіту інструменту"""
    data       = request.get_json()
    instrument = (data.get('instrument') or '').strip()
    report     = (data.get('report') or '').strip()
    stance     = (data.get('stance') or 'neutral').strip()

    if not instrument or not report:
        return jsonify({'error': 'instrument і report обовʼязкові'}), 400
    if stance not in ('long', 'short', 'neutral'):
        return jsonify({'error': 'stance має бути: long / short / neutral'}), 400

    with get_db() as db:
        db.execute("""
            INSERT INTO rpt_stances (user_id, instrument, report, stance, updated)
            VALUES (?,?,?,?, CURRENT_TIMESTAMP)
            ON CONFLICT(user_id, instrument, report)
            DO UPDATE SET stance=excluded.stance, updated=CURRENT_TIMESTAMP
        """, (session['user_id'], instrument, report, stance))
    return jsonify({'ok': True})

@app.route('/api/rpt_stances', methods=['GET'])
@login_required
def get_rpt_stances():
    """Повертає всі stances для одного інструменту"""
    instrument = request.args.get('instrument', '').strip()
    if not instrument:
        return jsonify({'error': 'instrument обовʼязковий'}), 400

    with get_db() as db:
        rows = db.execute(
            "SELECT report, stance FROM rpt_stances WHERE user_id=? AND instrument=?",
            (session['user_id'], instrument)
        ).fetchall()

    return jsonify({'stances': {r['report']: r['stance'] for r in rows}})

@app.route('/api/rpt_stances/all', methods=['GET'])
@login_required
def get_all_rpt_stances():
    """Повертає всі stances поточного юзера (для initial load)"""
    with get_db() as db:
        rows = db.execute(
            "SELECT instrument, report, stance FROM rpt_stances WHERE user_id=?",
            (session['user_id'],)
        ).fetchall()

    # Групуємо: {instrument: {report: stance}}
    result = {}
    for r in rows:
        inst = r['instrument']
        if inst not in result:
            result[inst] = {}
        result[inst][r['report']] = r['stance']
...
.field label{display:block;font-size:9px;color:var(--d);letter-spacing:.8px;margin-bottom:5px;}
...
<div class="box">
  ...
  <div id="t-login">
    <div class="field"><label>EMAIL</label>
      <input type="email" id="l-email" placeholder="your@email.com"></div>
  ...
  <div id="t-reg" style="display:none">
    <div class="field"><label>EMAIL</label>
      <input type="email" id="r-email" placeholder="your@email.com"></div>
    <div class="field"><label>ПАРОЛЬ</label>
      <input type="password" id="r-pass" placeholder="мінімум 6 символів"></div>
    <div class="field"><label>ПАРОЛЬ ЩЕ РАЗ</label>
...
function showMsg(id,text,isErr){
  ...
  el.textContent=text; el.className='msg '+(isErr?'err':'ok'); el.style.display='block';
}
...
async function doRegister(){
  ...
  const res=await fetch('/api/register',{method:'POST',
    ...
    body:JSON.stringify({email,password:pass})});
  const data=await res.json();
  if(data.ok){showMsg('r-msg','Успішно! Тепер увійдіть.',false);
    setTimeout(()=>showTab('login'),1500);}
  else{showMsg('r-msg',data.error||'Помилка',true);}
}
</script>
</body>
</html>

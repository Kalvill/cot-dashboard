...
CHART_WEEKS  = 260   # 5 років max для графіку
SPARK_WEEKS  = 26    # sparkline у картках
HISTORY      = 52    # рядків у таблиці
...
def fv(n, short=False, sign=False):
    """Форматує число. short=True → K/M. sign=True → завжди +/- для нуля не ставить."""
    ...
    if short:
        ...
        # повне число з пробілом як роздільник тисяч
...
def fv_full(n, sign=False):
    """Повне число для таблиці: +78 560"""
    try: n = int(round(float(n)))
    except: return '—'
    body = f"{abs(n):,}".replace(',', '\u202f')
    if n > 0:   return ('+' if sign else '') + body
...
# ================================================================
# 🎨  SVG SPARKLINE (генерується в Python)
# ================================================================
def make_sparkline(series, h=38):
    """SVG area-chart для metric card. Показує останні SPARK_WEEKS тижнів."""
...
def read_sheet(xl, name, overview):
    try:
        ...
        # ── Дані для графіку (до CHART_WEEKS тижнів) ──────
        ...
        hist = {
            ...
            'ls_pct': norm_pct(gch(COL['ls_pct'])), 'ls_net': gch(COL['ls_net']),
            'cm_cl':  gch(COL['cm_cl']),  'cm_cs': gch(COL['cm_cs']),
            'cm_pct': norm_pct(gch(COL['cm_pct'])), 'cm_net': gch(COL['cm_net']),
            'st_cl':  gch(COL['st_cl']),  'st_cs': gch(COL['st_cs']),
            'st_pct': norm_pct(gch(COL['st_pct'])), 'st_net': gch(COL['st_net']),
            'oi':     gch(oi_col),
        }
        ...
        return {
            ...
            # Sparkline series (останні SPARK_WEEKS)
            'spark': {
                'ls': ls_net, 'cm': cm_net, 'st': st_net, 'oi': oi_all
            },
            'cur': {
                'date':   all_dates[i0],
...
# ================================================================
# 🎨  КОЛЬОРОВА ІНТЕНСИВНІСТЬ ДЛЯ ТАБЛИЦІ
# ================================================================
def intensity_bg(val, max_abs):
    ...
    try:
        ...
        op = 0.12 + ratio*0.65  # min 12%, max 77%
...
# ================================================================
# 🃏  ТАБЛИЦЯ (повні числа, колірна інтенсивність)
# ================================================================
def make_hist_table(hist):
    ...
    for i in range(n-1, -1, -1):
        ...
        def td_pct(v):
            return f'<td class="{cc(v)}">{fp(v, signed=True)}</td>'
    ...
    return (
        ...
        '<thead>'
        '<tr>'
        '<th rowspan="2" class="th-left">ДАТА</th>'
        '<th colspan="4" class="th-group">LARGE SPECULATORS</th>'
        '<th colspan="4" class="th-group">COMMERCIALS</th>'
        '<th colspan="4" class="th-group">SMALL TRADERS</th>'
        '<th class="th-group">OI</th>'
        '</tr>'
        '<tr>'
        '<th>CHG L</th><th>CHG S</th><th>% NET</th><th class="sep-r">NET POS</th>'
        '<th>CHG L</th><th>CHG S</th><th>% NET</th><th class="sep-r">NET POS</th>'
        '<th>CHG L</th><th>CHG S</th><th>% NET</th><th class="sep-r">NET POS</th>'
        '<th></th>'
        '</tr>'
...
def make_instrument_view(d):
    ...
    # ── Metric cards ─────────────────────────────────────
    mc_ls = make_metric_card(
        'LARGE SPEC (NETTO)', c['ls_net'],
        f'{ar(c["ls_chg"])} {fv(abs(int(c["ls_chg"])), True)}',
        cc(c['ls_chg']),
        f'COT Index: {fp(d["cot_idx"]["ls"]["all"])}',
        d['spark']['ls']
    )
    mc_cm = make_metric_card(
        'COMMERCIALS (NETTO)', c['cm_net'],
        f'{ar(c["cm_chg"])} {fv(abs(int(c["cm_chg"])), True)}',
        cc(c['cm_chg']),
        f'COT Index: {fp(d["cot_idx"]["cm"]["all"])}',
        d['spark']['cm']
    )
    mc_st = make_metric_card(
        'SMALL TRADERS (NETTO)', c['st_net'],
        f'{ar(c["st_chg"])} {fv(abs(int(c["st_chg"])), True)}',
        cc(c['st_chg']),
        f'COT Index: {fp(d["cot_idx"]["st"]["all"])}',
        d['spark']['st']
    )
    mc_oi = make_metric_card(
        'OPEN INTEREST', c['oi'],
        f'{ar(c["oi_pct"])} {fp(c["oi_pct"])}',
        cc(c['oi_pct']),
        f'зміна: {fv(int(c["oi_chg"]), True, sign=True)}',
        d['spark']['oi']
    )
    ...
    def analysis_row(lbl, net, cl, cs, chg):
        ...
        return (
            ...
            f'<span class="arow-net">{fv(net, sign=True)}</span></div>'
            f'<div class="arow-chg-row">'
            f'<span class="achg-item"><span class="achg-lbl">CHG L</span>'
            f'<span class="{cc(cl)}">{fv(cl, True, sign=True)}</span></span>'
            f'<span class="achg-item"><span class="achg-lbl">CHG S</span>'
            f'<span class="{cc(-cs)}">{fv(-cs, True, sign=True)}</span></span>'
            f'<span class="achg-item"><span class="achg-lbl">ΔNet</span>'
            f'<span class="{cc(chg)}">{fv(chg, True, sign=True)}</span></span>'
            f'</div>'
            f'</div>'
        )
    ...
    pct_panel = (
        ...
        f'<div class="plbl">ПЕРЦЕНТИЛЬ ПОЗИЦІЇ (COT INDEX)</div>'
        ...
        f'</div>'
        f'</div>'
        f'<div class="pbar-lb"><span>0% екстр.шорт</span><span>50%</span><span>100% екстр.лонг</span></div>'
        f'</div>'
        f'<script>_ci["{s}"]={cot_json};</script>'
    )

    # ── Графік з вибором горизонту ───────────────────────
    ...
    chart_block = (
        ...
        f'<span><span class="ll" style="background:transparent;border-top:2px dashed #4a9eff;height:0;display:inline-block;vertical-align:middle;width:14px;margin-right:4px"></span>Small Traders</span>'
        f'</div></div>'
        f'</div>'
...
def generate_html(data):
    ...
    for cat, instruments in CATEGORIES.items():
        ...
        cat_sects.append(
            ...
            f'<div class="itabs">{inst_btns}</div>'
    ...
    return (
        ...
        + f'</div>'
        + '<div class="ctabs">' + ''.join(cat_tabs) + '</div>'
        + ''.join(cat_sects)
...
/* ── HEADER ── */
.hdr{padding:12px 24px;background:var(--bg2);border-bottom:1px solid var(--bd);
...
.itabs{display:flex;gap:4px;padding:7px 24px;background:var(--bg);
  border-bottom:1px solid var(--bd);flex-wrap:wrap;
  position:sticky;top:93px;z-index:180;}
...
/* ── REPORT BUTTONS ── */
.report-tabs{display:flex;align-items:center;gap:6px;margin-bottom:14px;}
...
.mc{background:var(--bg2);border:1px solid var(--bd);border-radius:5px;padding:14px 16px;overflow:hidden;}
.mc-lbl{font-size:9px;color:#fff;letter-spacing:.5px;margin-bottom:6px;}
.mc-val{font-size:26px;font-weight:bold;letter-spacing:.3px;}
...
.arow-dir{font-size:13px;font-weight:bold;margin-bottom:5px;}
.arow-net{font-size:11px;margin-left:8px;opacity:.8;font-weight:normal;}
.arow-chg-row{display:flex;gap:10px;flex-wrap:wrap;}
.achg-item{display:flex;gap:4px;align-items:center;}
.achg-lbl{font-size:8px;color:var(--d);}
...
.ptick{position:absolute;top:0;width:2px;height:100%;background:rgba(255,255,255,.2);}
...
.chart-leg{display:flex;gap:12px;font-size:10px;color:var(--d);}
...
table.ht .date-col{text-align:left;color:var(--d);}
...
HTML_FOOT = """
<script>
const CurPer = {};  // sid -> nWeeks для синхронізації
...
/* ── Головний лінійний графік ── */
function drawMainChart(sid, nWeeks) {
  const cv=document.getElementById('cv_'+sid);
  if(!cv) return;
...
/* ── COT Index перцентиль: вибір учасника ── */
...
def main():
    print()
    print("="*55)
    print("   COT Dashboard Generator v6")
    print("="*55)
    print()
